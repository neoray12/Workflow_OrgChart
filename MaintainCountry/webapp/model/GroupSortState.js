sap.ui.define([
	...
	return BaseObject.extend("saptw.MaintainCountry.model.GroupSortState", {
		 ...
		 * @class
		 * @public
		 * @alias saptw.MaintainCountry.model.GroupSortState
		 */
		...
		/**
		 * Sorts by countryName, or by countryId
		 *
		...
		/**
		 * Groups by countryId, or resets the grouping for the key "None"
		 *
		 * @param sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "countryId") {
				...
				this._oViewModel.setProperty("/sortBy", "countryId");

				aSorters.push(
					new Sorter("countryId", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "countryName");
			}
