sap.ui.define([
	...
	return BaseObject.extend("saptw.MaintainDepartment.model.GroupSortState", {
		 ...
		 * @class
		 * @public
		 * @alias saptw.MaintainDepartment.model.GroupSortState
		 */
		...
		/**
		 * Sorts by description, or by departmentId
		 *
		...
		/**
		 * Groups by departmentId, or resets the grouping for the key "None"
		 *
		 * @param sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "departmentId") {
				// Grouping means sorting so we set the select to the same Entity used for grouping
				this._oViewModel.setProperty("/sortBy", "departmentId");

				aSorters.push(
					new Sorter("departmentId", false,
